...
numberOfAvrageingPoints=7;
TotalNumberOfIterationsGamma=20;
finalValueOfR0=100;
numberOfR0s=20;
profile on;

R0=linspace(1,finalValueOfR0,numberOfR0s);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ibeta=1:2
  beta=0.3+0.5*(ibeta-1);
  for iterationConstantR0=1:numberOfR0s
    gamma=beta/R0(iterationConstantR0);
    for avragerun=1:numberOfAvrageingPoints
      ...
      Rplot(iterationConstantR0,1,ibeta)=R0(iterationConstantR0);
      Rplot(iterationConstantR0,2,ibeta)=Rinf/numberOfAvrageingPoints+Rplot(iterationConstantR0,2,ibeta);
...
title('d=0.8, 1% infected initially, avrage over 7 runs, 1000 agents')
legend('\beta=0.3','beta=0.8')
profile viewer